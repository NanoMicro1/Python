...
enemy_y_position = 0
random_x_position = random.randint(1, 480) # x좌표의 랜덤으로 뽑아내기
enemy_x_position = random_x_position

# y좌표는 흐르게 놔둬야 하는데 시간으로 이걸 표현해야 하나?
...
# 20 fps : 1초 동안에 20번 동작 -> 1번에 5만큼 이동한다는 뜻. 5 * 20 = 100
    ...
    # 적의 떨어짐. 낙하
    enemy_fall_speed = 400 * (dt / 1000)
    enemy_y_position += enemy_fall_speed
    ...
    # enemy 경계값 처리. 일단 바닥에 닿으면 사라지네..? 뭐지?
    if enemy_y_position < 0:
       enemy_y_position = 0
    elif enemy_y_position > screen_height - enemy_height:
         enemy_y_position = screen_height #- enemy_height
    ...
    if character_rect.colliderect(enemy_rect):
        ...
        running = False
    elif character_rect.colliderect(enemy_rect) == False:
         enemy_y_position += 0
         random_x_position = random.randint(1, 480) # x좌표의 랜덤으로 뽑아내기
         enemy_x_position = random_x_position
         #screen.blit(enemy, (enemy_x_position, enemy_y_position)) # enemy 그리기
